...
#include <acados_solver_ocp_quadrotor_nmpc_ros1_100hz_e03501ad.h>
...
constexpr int kNu = 4;
constexpr int kNp = 7;
constexpr int kN = OCP_QUADROTOR_NMPC_ROS1_100HZ_E03501AD_N;
...
class AcadosController {
 public:
  AcadosController(double mass, double gravity, double sample_time, double tmin,
    ...
    capsule_ = ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_create_capsule();
    if (!capsule_ || ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_create_with_discretization(
          capsule_, kN, nullptr) != ACADOS_SUCCESS) throw std::runtime_error("acados create failed");
    config_ = ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_get_nlp_config(capsule_);
    dims_ = ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_get_nlp_dims(capsule_);
    in_ = ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_get_nlp_in(capsule_);
    out_ = ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_get_nlp_out(capsule_);
  }
  ~AcadosController() {
    if (capsule_) {
      ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_free(capsule_);
      ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_free_capsule(capsule_);
    }
  }
  ...
  bool solve(const State &state, const States &refs, const Controls &ff,
    ...
    for (int i=0; i<kN; ++i) {
      ...
      ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_update_params(capsule_, i, p.data(), kNp);
    ...
    ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_update_params(capsule_, kN, p.data(), kNp);
    std::array<double,9> yt{}; for (int j=0;j<6;++j) yt[j]=refs(j,kN);
    ocp_nlp_cost_model_set(config_,dims_,in_,kN,"yref",yt.data());
    const int status = ocp_quadrotor_nmpc_ros1_100hz_e03501ad_acados_solve(capsule_);
 ...
 private:
  using Capsule = ocp_quadrotor_nmpc_ros1_100hz_e03501ad_solver_capsule;
...
class Node {
 public:
  Node() : nh_(), pnh_("~"), controller_(param("mass",2.0),param("gravity",9.80665),
    ...
    const std::string root=param("flight_log_root",std::string("/home/ljt/nmpc_log"));
 ...
 private:
  ...
  void enable_callback(const std_msgs::Bool::ConstPtr &m) {
    ...
    if (enabled_ && !was_enabled) { control_enable_time_=stamp_seconds(Clock::now()); eso_active_=false; last_disturbance_.setZero(); }
    if (!enabled_) { controller_.reset(); eso_active_=false; request_land(); }
  }
  void state_callback(const mavros_msgs::State::ConstPtr &m) {
    std::lock_guard<std::mutex> lock(mutex_); fcu_connected_=m->connected; fcu_armed_=m->armed; fcu_mode_=m->mode; last_state_rx_=stamp_seconds(Clock::now());
  }
  ...
  void trajectory_callback(const eso_nmpc_cpp::NmpcTrajectorySetpoint::ConstPtr &m) {
    ...
    std::lock_guard<std::mutex> lock(mutex_); trajectory_=t; trajectory_valid_=true; fallback_trajectory_=false; trajectory_rx_=stamp_seconds(Clock::now());
  }
  ...
  void odom_callback(const nav_msgs::Odometry::ConstPtr &m) {
    ...
    {
      std::lock_guard<std::mutex> lock(mutex_);
      const bool offboard=fcu_mode_==offboard_mode_;
      // Before pilot-selected OFFBOARD, continuously follow the measured pose.  On entry to
      // OFFBOARD the last pose becomes the fixed hover reference instead of the startup pose.
      if (!trajectory_valid_ || (fallback_trajectory_ && !offboard)) {
        t.sample_time=dt_;
        for(int i=0;i<kPoints;++i) {
          for(int j=0;j<3;++j) { t.position[3*i+j]=s[j]; t.velocity[3*i+j]=0.0; t.acceleration[3*i+j]=0.0; }
          t.yaw[i]=std::atan2(2.0*(s[6]*s[9]+s[7]*s[8]),1.0-2.0*(s[8]*s[8]+s[9]*s[9]));
        }
        trajectory_=t; trajectory_valid_=true; fallback_trajectory_=true; trajectory_rx_=stamp_seconds(rx);
      }
      if (!fallback_trajectory_ && stamp_seconds(rx)-trajectory_rx_>timeout_) return;
      t=trajectory_;
    }
    const double receive=stamp_seconds(rx);
    double dt=dt_;
    if(last_odom_time_>0.0) dt=std::clamp(receive-last_odom_time_,1e-3,0.1);
    last_odom_time_=receive;
    ...
    bool use_rc=false;
    {
      ...
      const bool offboard=fcu_mode_==offboard_mode_;
      const bool aux=offboard && rc_aux_channel_>0 && rc_aux_enabled_;
    ...
    if(!controller_.solve(s,refs,ff,disturbance,cmd)) { publish_hold(); request_land(); return; }
    const auto solve1=Clock::now(); last_command_thrust_=cmd[0]; publish(cmd); const auto pub=Clock::now(); last_cmd_=cmd;
    ...
    timing_<<std::setprecision(12)<<stamp_seconds(pub)<<","<<sample_age_ms<<","<<1e3*std::chrono::duration<double>(pub-rx).count()<<","<<1e3*std::chrono::duration<double>(solve1-solve0).count()<<","<<s[0]<<","<<s[1]<<","<<s[2]<<","<<refs(0,0)<<","<<refs(1,0)<<","<<refs(2,0)<<","<<referr<<","<<disturbance[0]<<","<<disturbance[1]<<","<<disturbance[2]<<","<<cmd[0]<<","<<cmd[1]<<","<<cmd[2]<<","<<cmd[3]<<","<<normalized_throttle<<","<<(use_rc?1:0)<<","<<(eso_active_?1:0)<<","<<(estimator_ready_?1:0)<<","<<(shadow_mode_?1:0)<<","<<(enabled_&&!shadow_mode_&&estimator_ready_?1:0)<<"\n";
    timing_.flush();
  }
  void request_land() {
    if (shadow_mode_ || !auto_manage_flight_ || !fcu_armed_) return;
  ...
  void flight_tick() {
    if(shadow_mode_ || !auto_manage_flight_ || !enabled_ || !estimator_ready_) return;
    const double now=stamp_seconds(Clock::now());
    ...
    if(phase_==WAIT) { phase_=PRESTREAM; phase_started_=now; ROS_INFO("NMPC prestream started"); }
    if(phase_==PRESTREAM && now-phase_started_>=prestream_s_) { phase_=OFFBOARD; phase_started_=now; }
    ...
    if(phase_==OFFBOARD) {
      ...
      if(mode==offboard_mode_) { phase_=ARMING; phase_started_=now; }
  ...
  void publish(const Eigen::Vector4d &c){
    if(shadow_mode_||!estimator_ready_)return;
  ...
  enum FlightPhase { WAIT, PRESTREAM, OFFBOARD, ARMING, FLIGHT, LANDING, FINISHED };
  ...
  Trajectory trajectory_{}; bool trajectory_valid_{false},fallback_trajectory_{false},enabled_{false},shadow_mode_{true};
  ...
  double trajectory_rx_{0},last_state_rx_{0},last_odom_time_{0},mass_{},gravity_{},dt_{},timeout_{}; int horizon_{};
  ...
  bool auto_manage_flight_{false}; double prestream_s_{1.5},service_timeout_s_{8},state_timeout_s_{1},auto_land_after_s_{0},phase_started_{0},last_service_call_{0}; std::string offboard_mode_; FlightPhase phase_{WAIT}; std::string fcu_mode_;
